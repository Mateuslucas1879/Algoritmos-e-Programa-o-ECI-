class Sudoku:
    ...
    def resolver(self,indice_zero=0):
        if indice_zero == len(self.zeros):
            return True
        i, j = self.zeros[indice_zero]
        candidatos = self.obter_candidatos(i,j)

        if not candidatos:
            return False
        for palpites in candidatos:
            self.tabuleiro[i][j] = palpites
            if self.resolver(indice_zero + 1):
                return True
    ...
    def __repr__(self):
        ...
        for i , linha in enumerate(self.tabuleiro):
            ...
            for j , valor in enumerate(linha):
                if j % 3 == 0 and j != 0:
                    elemento.append("|")
                elemento.append(str(valor) if valor != 0 else '.')
            linhas.append("".join(elemento))
        return "\n".join(linhas)
